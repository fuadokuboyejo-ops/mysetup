...
export default function OnboardingOpenerScreen({ onContinue }) {
  const spin = useRef(new Animated.Value(0)).current;
  const breathe = useRef(new Animated.Value(0)).current;
  const rise = useRef(new Animated.Value(0)).current;
  // Rebuilt if the piece list changes (keeps Fast Refresh from leaving a stale
  // shorter array behind, which would read undefined for the new pieces).
  const bobsRef = useRef([]);
  if (bobsRef.current.length !== PIECES.length) {
    bobsRef.current = PIECES.map(() => new Animated.Value(0));
  }
